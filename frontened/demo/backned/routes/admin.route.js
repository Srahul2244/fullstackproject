...
require('dotenv').config()
 ...
 adminRouter.post("/adminSign",(req,res)=>{
...
adminRouter.get("/admin/fetch",async(req,res)=>{
    ...
    try{
    const data =await mediaModel.find().sort({price:sortBy}).skip(skip).limit(_limit)
    res.send({data:data})
...
adminRouter.get("/single/:id", async (req, res) => {
    const { id } = req.params;
    try {
      const product = await mediaModel.find({ _id: id });
...
//   .....post request to add products in Alldata
  adminRouter.post("/addproduct", async (req, res) => {
    ...
    try {
      const newProduct = new mediaModel(payload);
...
//   ...patch request for admin
  ...
  adminRouter.patch(`/updateproduct/:id`, async (req, res) => {
    ...
    const payload = req.body;
    try {
      const updatedProduct = await mediaModel.findByIdAndUpdate(
        { _id: ID },
        payload
      );
...
adminRouter.get("/allUserOrder", async (req, res) => {
  try {
    const Order = await AdminModel.find({});
